...
setwd("C:/Users/Downloads/GitHub")
...
#Subtract the MQ water blank value from the averages
...
#Fourth, compute the nmol P in the original digestion solution (3 ml in falcon tube)
lu_nmolp <-  well_nmolp * 15
...
sd_p_calcite  <- abs(((((((data.avg.true + data.sd - Intercept) / Slope) *  15 * 10^-3)/ (as.numeric(dissolved[1,]))) / (1000/100.00869)) - mmolp_molcalc) / sqrt(n))
sd_p_dolomite <- abs(((((((data.avg.true + data.sd - Intercept) / Slope) *  15 * 10^-3 )/(as.numeric(dissolved[1,])))/ (1000/184.401)) - mmolp_moldol) / sqrt(n))   
